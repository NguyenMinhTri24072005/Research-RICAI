...
def _load_or_create_ca(cert_dir: Path):
    try:
        ...
        raise RuntimeError(
            "Thiếu thư viện cryptography để tạo HTTPS. Hãy cài: pip install cryptography."
        ) from exc

    key_path = cert_dir / "rice-vision-ca-key.pem"
    cert_path = cert_dir / "rice-vision-ca.pem"
    install_path = cert_dir / "RiceVision-CA.cer"
    ...
    subject = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Rice Vision AI"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Rice Vision AI Local CA"),
        ]
    )
    now = _utc_now()
    certificate = (
...
def ensure_local_certificates(cert_dir: Path, addresses: Iterable[str]) -> TlsMaterial:
    ...
    subject = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Rice Vision AI"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Rice Vision Local Server"),
        ]
    )
    ...
    server_key_path = cert_dir / "rice-vision-server-key.pem"
    server_cert_path = cert_dir / "rice-vision-server.pem"
    _write_file(
        server_key_path,
