...
cells_cnn = [
    make_cell('markdown', '''# 🌾 MODULE 3: HUẤN LUYỆN MÔ HÌNH PHÂN LOẠI CHẤT LƯỢNG HẠT LÚA (DENSENET121)
...
### 🎯 Mục tiêu & Các cải tiến cốt lõi trong file:
1. **Nâng cấp độ phân giải**: Kích thước ảnh chuẩn $224 \times 224$ (chuẩn của DenseNet121) để giữ lại chi tiết vết nứt, thâm đầu phôi.
2. **Tăng cường dữ liệu (Data Augmentation)**: Tự động xoay $360^\circ$, lật ảnh, đổi tương phản để mô hình học bất biến góc chụp.
3. **Cân bằng trọng số lớp (Class Weights)**: Tự động tính toán mức phạt để cân bằng tỷ lệ mẫu (hạt nguyên vs hạt khuyết tật).
4. **Chiến lược Fine-Tuning 2 giai đoạn**: Warmup Classifier Head $\to$ Mở khóa 30 layers cuối của DenseNet121 với Learning Rate nhỏ ($10^{-5}$).
5. **Cơ chế chống Overfit**: Tích hợp `EarlyStopping`, `ReduceLROnPlateau`, `ModelCheckpoint` để luôn lưu trọng số tối ưu nhất.
6. **Đánh giá kiểm nghiệm khoa học**: Đánh giá trên tập Test độc lập, xuất Ma trận nhầm lẫn (Confusion Matrix) và Báo cáo F1-Score chi tiết.'''),
...
# Đường dẫn folder chứa các thư mục nhãn (ví dụ: hat_nguyen, hat_khuyet_tat)
DATASET_DIR = os.path.join(BASE_PATH, "DATASETS/CNN_RICE_DATASET_2CLASSES")
...
# 1. Khối tăng cường dữ liệu hạt lúa (Xoay tự do 360 độ, lật ngang/dọc, chỉnh tương phản)
...
print("🚀 Bắt đầu Giai đoạn 1: Huấn luyện Classifier Head (15 Epochs)...")
history_phase1 = model.fit(
    ...
    make_cell('code', '''# Bước 6: GIAI ĐOẠN 2 — Fine-Tuning các tầng sâu của DenseNet121
# Mở khóa 30 layers cuối cùng của DenseNet
base_model.trainable = True
for layer in base_model.layers[:-30]:
    layer.trainable = False

# Sử dụng Learning Rate rất nhỏ để tinh chỉnh
model.compile(
...
print("🚀 Bắt đầu Giai đoạn 2: Fine-Tuning các tầng sâu (25 Epochs)...")
