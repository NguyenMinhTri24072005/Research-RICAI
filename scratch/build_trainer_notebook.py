...
def create_notebook():
    nb_path = Path("LINEAR_REGRESSION_MODEL/RICE_SEED_REGRESSION_TRAINER.ipynb")
    ...
    def add_code(source, cell_id):
        # Ensure source lines end with newline except possibly the last
    ...
    # -------------------------------------------------------------------------
    # Cell 0: Markdown Intro & TOC
    # -------------------------------------------------------------------------
    add_md("""# 🌾 RICE VISION AI — BỘ HUẤN LUYỆN & SO SÁNH HỒI QUY ĐA MÔ HÌNH (31 ĐẶC TRƯNG)
...
""", "intro")
    ...
    # -------------------------------------------------------------------------
    # Cell 1: ENV_SETUP & IMPORTS
    # -------------------------------------------------------------------------
    add_code("""# Cell ID: ENV_SETUP
# ==============================================================================
# KHU 01: MÔI TRƯỜNG & KHAI BÁO THƯ VIỆN (ENV_SETUP & IMPORTS)
# ==============================================================================
...
REPO_ROOT = NOTEBOOK_DIR.parent if (NOTEBOOK_DIR.parent / "AI_SERVICES").exists() else NOTEBOOK_DIR
...
""", "RUN_CONFIG")
    ...
    # -------------------------------------------------------------------------
    add_code("""# Cell ID: FEATURE_CONTRACT
# ==============================================================================
...
""", "SPLIT_BUILD")
    ...
    # -------------------------------------------------------------------------
    add_code("""# Cell ID: PREPROCESSING
# ==============================================================================
...
def compute_metrics(
    ...
    Tính toán các chỉ số sai số chuẩn:
      - Không làm tròn, không cắt cụt (clamp) dự đoán trước khi tính.
      - Xử lý shape (n,).
...
""", "HELPERS")
    ...
    # -------------------------------------------------------------------------
    add_md("""## Khu 06: Cấu Hình 19 Mô Hình Ứng Viên (Model Inventory)
...
""", "MODEL_CONFIGS_HEADER")

    # -------------------------------------------------------------------------
    # Cell 10: MODEL_REGISTRY_INIT
    # -------------------------------------------------------------------------
    add_code("""# Cell ID: MODEL_REGISTRY_INIT
# ==============================================================================
...
"""
        add_code(code_text, cell_id)

    # -------------------------------------------------------------------------
    # Cell 30: TRAIN_ALL
    # -------------------------------------------------------------------------
    add_code("""# Cell ID: TRAIN_ALL
# ==============================================================================
...
for m_id, res in successful_models.items():
    ...
    pred_train = pipeline.predict(X_train_outer)
    pred_test = pipeline.predict(X_test_outer)
...
if hasattr(model_obj, "feature_importances_"):
    ...
    raw_weights = model_obj.coef_ / scaler_obj.scale_
...
if not EXPORT_BUNDLES:
    ...
    for m_id, ev in EVALUATION_RESULTS.items():
        ...
        if m_id in linear_whitelist and hasattr(model_inst, "coef_"):
            raw_w = (model_inst.coef_ / scaler_inst.scale_).tolist()
            raw_b = float(model_inst.intercept_ - np.sum((model_inst.coef_ * scaler_inst.mean_) / scaler_inst.scale_))
...
if not EXPORT_BUNDLES:
    ...
    for m_id, ev in EVALUATION_RESULTS.items():
        ...
        pred_pipeline = reloaded_pipe.predict(X_test_outer)
        pred_separate = reloaded_model.predict(reloaded_scaler.transform(X_test_outer))
...
""", "RELOAD_SMOKE")
    ...
    print(f"Created {nb_path} with {len(cells)} cells successfully.")
